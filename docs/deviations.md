...
## D270 ⚠ 💤 the Jd accessors read a payload-contiguous bit layout and the packers write a framed one, so neither can read the other's message

*Message and echo batch of 2026-08-11, from `V90Jd::getRatesMask` (blob 0x1e8b0), `getConstelationSize` (0x1e900), `getMaxLookahead` (0x1e920) and their `V92Jd` counterparts at 0x11eb0, 0x11f00, 0x11f20, plus `V92Jd::getJdPhase` (0x11e60). **Reachability: unmeasured** -- no caller of any of the seven has been read. Status: CONFIRMED present, both readings driven against the blob. Fix class: none proposed; reproduced, not reconciled.*

**Finding 1390.** The constructors and the packers put the rate mask at `bits[18..33]` and `bits[35..46]`, the constellation pair at `bits[47..48]` and the lookahead pair at `bits[49..50]`, around a 17-byte group frame; the accessors read `bits[0..27]`, `bits[28..29]` and `bits[30..31]`, with no frame at all, and `getJdPhase` reads `phaseBits[0..15]` where the constructor writes the Q16 phase at `phaseBits[18..33]`.  The two layouts differ by the framing and by 18 or 19 positions, so a Jd object cannot decode the message it just packed.  Both halves are reproduced exactly as the object has them and each is driven against the blob; `t_v90jd.cpp` and `t_v92jd.cpp` paint the payload positions explicitly and leave the rest of the vector on its varied seed, so a reconstruction reading one byte outside the payload fails rather than agreeing.
...
**Finding 1391.** Three of `V92Jd`'s four accessors are `V90Jd`'s instruction for instruction and read `bits`; this one reads +0x67 and +0x68, which in `V92Jd`'s map is the SECOND vector, and at one index higher than `V90Jd`'s reads in the first.  +0x67 is +0x1f plus the 0x48 that `phaseBits` displaces everything after it by.  In the framed layout those two bytes hold bits 11 and 12 of the Q16 phase, which is what the constructor writes there and what `packJdPhaseData` leaves alone.  Reproduced; the fixture paints the two vectors with different values so that reading the right index in the wrong vector diverges.
