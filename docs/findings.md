...
difference between a 64-bit and a 53-bit mantissa. It is the second x87
precision claim in this branch that turned out not to be observable on this
target -- the first is on `V90Phase2Info`'s scaled fraction, which lands
separately -- and both were settled by sweeping rather than by argument.
