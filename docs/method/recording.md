...
does the next one starting (finding 215). Embedding would have converted a
...
recorded as tiling, and the claim was not made (finding 630).
...
take a member pointer would change codegen to buy a spelling (finding 632).
...
(finding 636) — then all 48 mutation suites re-run, 2,874 mutations, every
verdict compared **by name**, zero changes (finding 638).

## Do not quote a number you cannot reproduce

One finding said "sixty-one of the seventy-nine were left". **Both numbers
were wrong and the ratio should not have been quoted at all** (finding 639).
...
spans became fields**, each of them named (finding 636).
...
> is stale is worse than no number, because it gets quoted (finding 545).
...
- **A new finding, when the commits carrying the error are pushed.** One entry
  ...
  own rule about reading codegen (finding 639).
- **A forward-pointing header, when the finding still needs reading.** Two
  ...
  top, with the original text left underneath, intact (findings 570 and 574,
  amended by findings 650 and 651).
...
- **A tier that could not be run is a gap, not a pass.** The codegen ratchet
  ...
  as a passing check (finding 554).
- **A prediction disproved is the measurement.** One prediction said eleven
  ...
  construction (finding 570). A later batch finally ran it: **eight flipped,
  and both predictions were wrong in opposite directions** (finding 651).
...
- **A detector that could not find the defect it was written for**, through
  four consecutive versions, each of which reported confidently (finding 618).
...
the word, and the remainder is read by hand (finding 670) — the two-pass
procedure finding 543 used, for the same reason. What is left in these files
...
and every measured count in the set (finding 543).
...
- **The checker walks *tracked* files.** A new document that has not been
  ...
  newest file is finding 540's skipped suite arriving in the deliverable
  (finding 670). Add the file, then check.
- **A miscitation in the record propagates into briefs.** One finding cites a
  ...
  this update relayed it onward as measured (finding 690). The record is more
