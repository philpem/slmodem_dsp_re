...
other** (finding F613), and each has a blindness that is structural rather than
...
## F1. Differential — the only thing that decides
...
- **A diagnostic call site that is absent.** The debug level ships at zero and
  ...
  functions that *are* reconstructed (finding F134).
...
- **Its own resolution.** Of 1,667 `diff_eq_int` call sites, **819 passed a
  ...
  disassembly (finding F220). Compare whole objects with a helper that
...
## F2. Mutation — anti-vacuity, not correctness
...
prints nothing. The test could not see call-site *order* at all (finding F149).
...
- **Its own losses.** An UNUSABLE mutation does not fail a run, so a suite that
  has quietly lost mutations still prints `0 NOT caught` (finding F347). Read
...
- **Where its anchors landed.** Nine entries in one suite were mutating a
  ...
  CAUGHT** — at a claim nobody made (finding F432). `src.count(find) == 1`
...
- **That its own equivalence arguments still hold.** A mutation marked
  ...
  correct statement about the object (finding F651, and §4 below).
...
existed only in the scrollback of whichever session last ran it (finding F545).
**37 is the figure at findings F545 and F638, and it has since moved**: eight of
`v34hstx1`'s eleven fell when the transcript tier gained its missing lines
(finding F651). No finding records a tree-wide total after that, so this one
...
CAUGHT → UNUSABLE; and an unusable mutation does not fail a run (finding F347),
...
## F3. Codegen against the period compiler
...
2005 (finding F606). A container with gcc-3.4.3 and binutils 2.15 compiles 76
of 81 translation units unmodified (finding F607).
...
|---|---|---|
| `-fno-pie` | zero `get_pc_thunk` in the blob, 41 in our build | finding F606 |
| `-fno-stack-protector` | no `__guard` symbol anywhere | finding F606 |
| `-fomit-frame-pointer` | `ulaw2alaw` opens `movzbl 0x4(%esp)`, ours `push %ebp` | size matches 6→29, identical 0→18 (finding F607) |
| `-maccumulate-outgoing-args` | `BwChDem_Delete` fills the outgoing area with `mov`, ours `push` | 29→37, 18→26 (finding F607) |
| `-mtune=i686` | nothing — it leaves no mark | 30→82 identical (finding F612) |
| `-frename-registers` | `Agc<float>::reset`, 7 instructions of 11 | 83→92 identical (finding F616) |
...
gives the same 82 and stays consistent with every byte (finding F612). Every
...
- **Free.** Register allocation. Instruction scheduling. Of the 22 same-size
  ...
  ordering or register allocation (finding F614).
...
- **The sharp version.** A reorder permitted by both sources is noise; **a
  ...
  search. None of the 22 showed one (finding F614).
- **Statement order is not preserved.** `toneiir_reset`'s source was already
  ...
  (finding F617). So the acceptance test for acting on a store-order hint is
...
- **grade 1 -- the instructions are the same and only register allocation
  ...
  register allocation is the compiler's free choice (finding F614) and chasing
...
byte-count ratio, not a mnemonic match, not an instruction count. Finding F7630
...
(finding F604). And relocated displacements, where the blob's addend rides
...
arithmetically (finding F616). Every one of `-O3`'s extra matches came from
...
*regression* then prints `ratchet OK -- gained` (finding F556). Read the number
...
(finding F651). Both halves are `gates.md` rule 2 and its third failure mode.

---

## F4. Transcript / diagnostics — not decoration
...
diagnostic could (finding F573).

### What this tier cannot see

Measured exhaustively, by applying each uncaught mutation and differencing one
side's transcript against itself (finding F570). Of eleven uncaught mutations,
...
- **Three are indistinguishable under the fixture's fill.** Both echo cancellers
  ...
  coefficients would** (finding F570).
- **Two print nothing at all.** `initdigital` emits no line on that path, so
  ...
  unobservable (finding F570).
- **One is a genuine oracle gap** — a transfer with no oracle, confirmed by
  name rather than inferred from "one survived" (finding F570).

### That measurement was of a tree that has since changed

Finding F570's method is sound: dump side A's transcript under each uncaught
...
remain — because three of the six that finding F570 called identical are
separable now, all three being swaps between two echo cancellers that had both
printed `Nothing to report` under the fixture it had (finding F651). The
prediction being tested said one would remain and finding F570 said six; both
...
The three that remain are the ones finding F570 called identical for reasons
...
value the code does not *use* and the trace now *reports* (finding F651).
...
> observable (finding F651). Every `why` in every suite is a hostage to that;
...
- **It needs a green baseline to run at all.** Turning the diagnostics on made
  ...
  for *did not exist* (finding F570). **That was the state at finding F570 and is
  no longer true** — the suite is green with the diagnostics on, the driver
  enables them unconditionally and the phase gate runs it (finding F651). The
...
- **The direction matters more than the count.** Of those 322 failures,
  **161 were `diagnostic lines` and 161 `transcript`, and not one was
  ...
  trace (finding F570). When the repair went in, spurious went 0 → 4 → 0, and
  those four *were* the defect (findings F571 and F573). The argument was made a
  ...
  cases short and said nothing about why (finding F650). Count both directions;
...
dead**: no caller anywhere in `src/` (finding F572).

Finding F574 read the other 73% as *behaviour* nobody had written, and **that
...
what was missing was the reporting threaded through them (finding F650, which
closes finding F574 and says so in its header). The honest statement is
...
passing run, 161 with it (finding F570). Whoever closes part of the gap watches
...
byte-identical in all 272 step cases throughout (finding F650). A number built
as a measurement, and taken to zero, is the strongest argument available for
building one.

Taking it to zero is also what disproved finding F574's reading of the gap, two
...
> produced **two missing lines and two spurious ones** (finding F650).

## F5. Spec conformance -- the only oracle that can indict the OBJECT
