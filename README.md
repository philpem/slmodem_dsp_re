...
The live phase ledger, current measured coverage, remaining objectives and
historical completion record now live together in
**[docs/remaining.md](docs/remaining.md)**. In short: every `.text` and `.data`
symbol in the reference object has a source reconstruction; the open project
milestones are validation/refinement, end-to-end V.90 digital-side bring-up,
and the separate 8 kHz retarget.
