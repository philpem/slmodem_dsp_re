...
| `vbt-chanshim` | The shared C channel model — runs in d-modem's place for each end and joins them over loopback. Band limit, delay, noise, independent or burst loss (`CHAN_BURST`), smooth clock offset (`CHAN_SMOOTH_PPM`), `CHAN_SLIP`, and tilt. Carries selectable profiles under `VBT_PROFILE` (or `CHAN_LINE_MODEL` for harness compatibility). |
