 ...
 * (finding 340).  None of the four entry points takes a float by value, so the
...
main(void)
{
	...
	diff_begin("agc: construct, reset and the frozen default");
	{
		...
		for (i = 0; i < 8; i++) {
			 ...
			 * caller defect in finding 348 is exactly this.
			 */
	 ...
	 * zero kills until built for deliberately (finding 348): the `>` guard
