 ...
 * setter is a no-op without `HARNESS_FLOAT_TOL` -- so this is exact there,
 * which is what keeps `make period` bit-for-bit.  Two NaNs compare equal.
 */
static int
...
mark_float_regions(void)
{
	...
	memcpy(&p, flo + 0x6bd0 + 0x24, sizeof p);	/* echoHistory */
	mark_float(p, 0, 0);

	memcpy(&arma, flo + 0x6bd0 + 0x04, sizeof arma);	/* arma */
	mark_float(arma, 0x2c, 2);
	if (arma) {
		memcpy(&p, arma + 0x0c, sizeof p);		/* m_yhist */
		mark_float(p, 0, 0);
	}

	memcpy(&demod, flo + OFF_DEMOD, sizeof demod);
	if (demod) {
		memcpy(&p, demod + 0x98, sizeof p);		/* coeffs */
		mark_float(p, 0, 0);
	}
