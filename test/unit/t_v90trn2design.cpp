...
run_design(void)
{
	...
	/*
	 * THE SHARED `slot` AT ZERO, which is the one value that separates
	 * `slot >= 0` from `slot > 0` and which the random sweep never
	 * produced.  It needs the descending arm to break on its LAST
	 * placement: two ucodes to place, so `slot` starts at 1, and a
	 * `topUcode` the walk reaches on the first one, so `top <= code`
	 * fires with `slot` already decremented to 0.  Designed rather than
	 * searched -- the table is linear, `dmin[k]` is zero for every phase
	 * so every phase takes that arm, and 11 is where the walk stops
	 * because the threshold is `dMin / 2` and dMin is 23.
	 */
	{
		V90Parameters *pp = (V90Parameters *)paramsOurs;
		int k, i;
		...
		/*
		 * A STEP AT `topUcode`, not a ramp.  The walk stops at the
		 * first entry reaching `dMin / 2`, and on a ramp that is far
		 * below the top, so `top <= code` never fires on the first
		 * placement and `slot` never reaches 0.  With everything below
		 * the top at 1 and the top itself at 1000, dMin is 566, the
		 * threshold is 283, and the walk runs all the way up to 11 --
		 * which is the top, so it breaks with `slot` at 0.
		 */
		for (k = 0; k < ROWS; k++) {
			for (i = 0; i < ROWLEN; i++) {
				tabUcode[k * ROWLEN + i] =
		...
		fill_pair(paramsOurs, paramsTheirs, 0x558 + GUARD, 21);
		pp->nofUcodesInTrn2 = 2;
		pp->maxUcode = 92;
		pp->unnamed_360 = 0;
		pp->SPECTRAL_SHAPER_SR = 3;
		pp->SPECTRAL_SHAPER_ID = 5;
		...
		fill_pair(mp2Ours, mp2Theirs, MP_BYTES + GUARD, 21);
		fill_pair(powerOurs, powerTheirs, 256, 22);
		dsplibs_debug_level = 0;
		set_designer(paramsOurs);
		((V90TRN2Designer *)designer)->power =
			(V90ConstellationPower *)powerOurs;
		...
		diff_eq_int("the slot-zero case returned", a, b, 0);
		diff_eq_obj("the slot-zero case designed", V90MappingParams,
			    mp2Ours, mp2Theirs, 0);
		/*
		 * The point of the case: `slot` reaches exactly 0 and the
		 * object treats that as a FAILURE.  A `> 0` test would return
		 * 1 here, so asserting the 0 is what makes the trial a
		 * separator rather than another passing row.
		 */
		diff_eq_int("slot reached zero and that is a failure", a, 0,
			    0);
		/*
		 * And the failure path really ran: what is left in the block
		 * is `setTrn2DummyConstel`'s descending run from 78, not a
		 * half-finished design.
		 */
		diff_eq_int("the dummy fill replaced the design, entry 0",
			    ((V90MappingParams *)mp2Ours)->constellation[0][0],
			    78, 0);
		diff_eq_int("the dummy fill replaced the design, entry 1",
			    ((V90MappingParams *)mp2Ours)->constellation[0][1],
			    77, 0);
	}

	/*
	 * THE RETRY CAP, approached from below.  The iterative arm gives up
	 * after 199 rounds, and a cap only shows in behaviour where a design
	 * SUCCEEDS later than the mutated cap and no later than the real one.
	 * Raising the scan's lower bound starves the search a little at a
	 * time, so the round count rises smoothly with it; sweeping the bound
	 * across its whole range is what puts some trial in that window.
	 */
	{
		V90Parameters *pp = (V90Parameters *)paramsOurs;
		int k, i, lo;
		...
		for (lo = 120; lo < 6000; lo += 11) {
			...
			/*
			 * A UNIT RAMP WITH A STEP ON TOP, and the step is what
			 * is swept.  The starting dMin is `top / (N - 0.5)`
			 * and the spacing the ramp can actually supply is 1,
			 * so the step sets how far dMin has to fall and
			 * therefore how many rounds the retry takes -- from
			 * one at the bottom of the sweep to past the cap at
			 * the top, which is what carries some trial through
			 * the window between a mutated cap and the real one.
			 */
			for (k = 0; k < ROWS; k++) {
				for (i = 0; i < ROWLEN; i++) {
					tabUcode[k * ROWLEN + i] =
			...
			((V90TRN2Designer *)designer)->power =
				(V90ConstellationPower *)powerOurs;
			a = our_trn2_design(designer, mp2Ours, tabUcode,
					    tabAlt, tabAllow, tabDmin, 1, 1,
					    0, tabTop, 9u, 7, 0);
			((V90TRN2Designer *)designer)->power =
				(V90ConstellationPower *)powerTheirs;
			b = ref_trn2_design(designer, mp2Theirs, tabUcode,
					    tabAlt, tabAllow, tabDmin, 1, 1,
					    0, tabTop, 9u, 7, 0);
			diff_eq_int("the cap sweep returned at step %ld", a,
				    b, lo);
		...
		diff_eq_int("the cap sweep designed on %ld steps",
			    sawDesign > 0, 1, sawDesign);
		diff_eq_int("the cap sweep gave up on %ld steps",
			    sawGiveUp > 0, 1, sawGiveUp);
	}
