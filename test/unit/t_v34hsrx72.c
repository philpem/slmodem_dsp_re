...
begin(short count)
{
	 ...
	 * i < rx->out_count; i++)` and writes `rx->timing_out[i]`, which is seven
 ...
 * it once per output, so `rx->out_count == 0` leaves the two equal and the sample
 ...
 * `flags`, `rx_blocks` and `out_count` are only 0x16 bytes past +0x10c, so a poke long
