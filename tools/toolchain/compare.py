...
AND THE VENDOR'S PATCH STACK NOW HAS A NUMBER TOO. The published
`ghcr.io/philpem/gcc-3.4.2-gentoo2005-docker:latest` image is Gentoo's own
`gcc-3.4.2-r2` built from its ebuild inside stage3-x86-2005.0, and it is the
only compiler here whose `.comment` matches the blob's byte for byte. It
changes NOTHING measurable: 182 of the
...
default is the published Gentoo image. Finding F2500.
