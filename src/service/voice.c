/*
 * voice.c -- the voice service's own TU.  The object's FILE symbols put the
 * symbols of this region in FOUR input files, in this order:
 *
 *     voice.c         0x0600-0x13b0   vce_*, VOICE_*, STRM_VCE_*
 *     fax.c           0x1450-0x2121   FAX_*  (the service dispatcher)
 *     rd.c            0x2130-0x2350   RD_create/delete/process/ring_details
 *     ringDetector.c  0x2360-0x2740   RingDetector_*  (five)
 *
 * `rd.c` and `ringDetector.c` are separate files here, matching those FILE
 * symbols (see src/service/rd.c and src/service/ringDetector.c).  Keeping
 * them separate is not cosmetic: with the ring detector in THIS translation
 * unit, O3 inlined `RingDetector_Delete` into RD_delete, `RingDetector_Process`
 * into RD_process and `RingDetector_GetLastRing` into both RD_process and
 * RD_ring_details, so three functions could not be byte-identical to the
 * object.  Split across translation units, the reference's CALL boundaries
 * come back and all three close (F11370).
 *
 * The `vce_*` / `VOICE_*` group below is in the object's own emission order:
 *
 ...
 * and docs/findings.md F10105/F10111.  The blob's FILE symbol for this group
 * is fax.c, so it is a further TU split that is not part of this change.
 */
