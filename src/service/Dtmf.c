...
/*
 * The DTMF receiver notch banks, moved here VERBATIM from the deleted
 * ours-only dtmf_coeffs.c: the blob defines them in Dtmf.c (F11412).  The
 * separate file existed only so their 68 float literals did not compete
 * with the logic anchors, which is a mutation-tier concern and not the
 * object.  A data symbol byte does not depend on its translation unit.
 */
/*
