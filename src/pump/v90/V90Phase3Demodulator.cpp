...
P3D_OFF(samplesInState,		0x02c, word2c);
P3D_OFF(eventCode,		0x030, eventcode);
			    ...
			    float float418Arg, unsigned int word14Arg)
{
	...
	eventCode = 0;
	pcmType = pcmTypeArg;
	timeoutBase = word14Arg;
	float_418 = float418Arg;
	samplesInState = word2cArg;
	state = stateArg;
 ...
 */
#define P3D_PHASE	((unsigned short)framePosition)
 ...
 */
#define P3D_LINMAPP(a, c)	((a)->linMapp[(unsigned short)framePosition][(c)])
#define P3D_LINMAPPALT(a, c)	((a)->linMappAlt[(unsigned short)framePosition][(c)])
...
/* `framePosition` runs 0,1,2,3,4,5,0 -- `inc; cmp $6; je; mov`. */
#define P3D_BUMP_FRAME() \
	do { \
		unsigned int next_ = framePosition + 1; \
		if (next_ == 6) \
			framePosition = 0; \
		else \
			framePosition = next_; \
 ...
 * program.  D321.
 *
 * `eventCode = 0` IS PER CASE AND NOT HOISTED.  Case 0 is the proof: it has no
...
V90Phase3Demodulator::getV90Decision(float sample)
{
	...
	case 0x00:
		...
		if (sdDetector->process(sample) > 0) {
			...
			samplesInState = 0;
			eventCode = 1;
		} else if (samplesInState == timeoutBase + 12000.0f) {
	...
	case 0x02:
		eventCode = 0;
		decision = s;
		if (samplesInState == 0x30) {
			if (quickConnect != 0) {
				state = (Phase3DemodulatorState)0x04;
				trn1dDdLength = (unsigned int)
	...
	case 0x05:
		...
		word_408 = (unsigned int)
		    autoDigitalImpDetector->studyUrefHandler(sample, framePosition);
		if (++framePosition == 6)
			framePosition = 0;
		if (word_408 == 2) {
			if (samplesInState % 6 == 0) {
	...
	case 0x06:
		...
		if (jd->unPackData(bit)) {
			if (samplesInState % 6 == 0 || quickConnect != 0) {
				if (DSPLIB_DEBUG_ON())
					dsplibs_debug_printf(
	...
	case 0x09:
		...
		if (samplesInState == timeoutBase + 38760.0f) {
	...
	case 0x0a:
		...
		if (phase3Modulator.usingSegmentLevel != 0) {
			if (autoDigitalImpDetector->isAltRbs((short)framePosition,
							     ucode, sample))
		...
		if (++framePosition == 6)
			framePosition = 0;
		if (samplesInState == (unsigned int)params->unnamed_30c) {
	...
	case 0x0b:
		...
		if (autoDigitalImpDetector->altRbsFlag[framePosition] != 0) {
			...
			if (P3D_ABS((int)symbol) != ucodeLevel)
				autoDigitalImpDetector
				    ->updateLinMappMeanAndVar((short)framePosition,
					P3D_CODE(P3D_ABS((int)symbol)));
			decision = (short)(P3D_SIGN(s)
			    * autoDigitalImpDetector
			      ->linMapp[P3D_PHASE]
		...
		if (samplesInState == (unsigned int)params->unnamed_314)
			eventCode = 0x0c;
		if (samplesInState == (unsigned int)params->unnamed_300) {
			if (short_400 != 0) {
				eventCode = 0x10;
		...
		if (++framePosition == 6)
			framePosition = 0;
		if (samplesInState == (unsigned int)params->unnamed_308) {
	...
	case 0x0c:
		...
		if (phase3Modulator.segmentPos == 0
			...
			autoDigitalImpDetector
			    ->uniteLinMappInfoOfUnsuspectedPhases(
				phase3Modulator.dilPcmCode);
		if (samplesInState == (unsigned int)params->unnamed_310) {
			autoDigitalImpDetector->porcessSecondStudy();
			samplesInState = 0;
			eventCode = 0x11;
			state = (Phase3DemodulatorState)0x10;
	...
	case 0x0f:
		...
		if (autoDigitalImpDetector->altRbsFlag[framePosition] != 0) {
			...
			decision = s;
			if (autoDigitalImpDetector->byte_280c[framePosition] == 0)
				decision = (short)(P3D_SIGN(s)
				    * autoDigitalImpDetector
				      ->prevLinMapp
					[P3D_CODE(P3D_ABS((int)symbol))]);
	...
	case 0x10:
		...
		if (samplesInState == (unsigned int)params->unnamed_318) {
			eventCode = 0x12;
		} else if (samplesInState == 0x9c40) {
			state = (Phase3DemodulatorState)0x18;
			samplesInState = 0;
	...
	case 0x1a:
		...
		if (sdDetector->process(sample) > 0) {
			...
			samplesInState = 0;
			eventCode = 0x37;
		} else if (samplesInState == (unsigned int)
	...
	case 0x1d:
		...
		decision = s;
		ansamToneDetector->process(sample);
		if (samplesInState == (unsigned int)
		    params->ANSPCM_DEMODULATION_LENGTH)
			eventCode = 0x3a;
		break;
 ...
 *
 *     samplesInState++;  eventCode = 0;  switch (state) { ... }  return decision;
 *
 * dispatched through a jump table of 34 entries at `.rodata+0x7cc` guarded by
 * `cmp $0x21,%eax; ja`.  `samplesInState` is the per-state sample counter -- every
 * arm that changes `state` also zeroes it -- and `eventCode` is an event code
...
V90Phase3Demodulator::getV92Decision(float sample)
{
	...
	case 10:				/* DILDemodFirstStudy */
		...
		if (mod->usingSegmentLevel != 0) {
			if ((short)adid->isAltRbs((short)framePosition, ucode,
						  sample) != 0) {
		...
		adid->calculateLinearMeanAndVar((short)level, (short)sym,
						framePosition);
		P3D_BUMP_FRAME();
		if (samplesInState == (unsigned int)V90PW(params)[P3D_P_30C]) {
	...
	case 11:				/* DILDemodSecondStudy */
		...
		if (adid->altRbsFlag[framePosition] != 0) {
			if (P3D_ABS(sym) == ucodeLevel) {
				...
				adid->addReceivedSampleToStorage(
				    (short)framePosition, (unsigned char)code,
				    sample);
			}
		} else {
			if (P3D_ABS(sym) != ucodeLevel) {
				code = P3D_CODE(P3D_ABS(sym));
				adid->updateLinMappMeanAndVar((short)framePosition,
							      (short)code);
			}
			code = P3D_CODE(P3D_ABS(sym));
			decision = (short)(P3D_SIGN(level) *
		...
		if (mod->segmentPos == 0 && mod->dilPcmCode != ucode)
			adid->uniteLinMappInfoOfUnsuspectedPhases(
			    mod->dilPcmCode);
		if (samplesInState == (unsigned int)V90PW(params)[P3D_P_314]) {
			eventCode = 0xc;
		} else if (samplesInState == (unsigned int)
		...
		P3D_BUMP_FRAME();
		if (samplesInState == (unsigned int)V90PW(params)[P3D_P_308]) {
	...
	case 12:				/* DILDemodThirdStudyStage */
		...
		if (mod->segmentPos == 0 && mod->dilPcmCode != ucode)
			adid->uniteLinMappInfoOfUnsuspectedPhases(
			    mod->dilPcmCode);
		if (samplesInState == (unsigned int)V90PW(params)[P3D_P_310]) {
			adid->porcessSecondStudy();
			samplesInState = 0;
			eventCode = 0x11;
			state = (Phase3DemodulatorState)0x10;
	...
	case 14:				/* DILDemodQCsecondStudy */
		...
		if (adid->altRbsFlag[framePosition] != 0) {
			...
			if (P3D_ABS(sym) == ucodeLevel) {
				...
				decision = (short)level;
				if (adid->byte_280c[framePosition] == 0) {
					code = P3D_CODE(P3D_ABS(sym));
					decision = (short)(P3D_SIGN(level) *
	...
	case 16:				/* DILDemodErrorRelaxation */
		...
		if (samplesInState == (unsigned int)V90PW(params)[P3D_P_318]) {
			eventCode = 0x12;
		} else if (samplesInState == 0x9c40) {
			state = (Phase3DemodulatorState)0x18;
			samplesInState = 0;
 ...
 * the same store, which is why the field is `jdNotRunLength` in both places.
 */
void
