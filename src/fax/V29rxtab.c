 ...
 * invent one.
 *
 * THE TRANSLATION UNIT.  These are the receiver's coefficient and gain
 * tables; the object names `V29rxtab.c` as its own FILE record between
 * `V29rxdec.c` and `V29tx.c`, and every table left here is referenced only
 * from `V29rx.c` / `V29r_prc.c` / `V29r_stc.c` functions.  Moving a table to
