/**
 * @file faxvmi.h
 * @brief Class 1 fax: the Virtual Modem Interface.
 *
 * FAXVMI wraps one modulation behind a numbered slot and puts a framing
 * layer in front of it. Two dispatches, and they are indexed by different
 * fields:
 *
 ...
 * That is evidence class 1 for the mode constants below: the author's
 * words, not an inference from what the code does.
 *
 * What is reconstructed here, and what is not: `FAXVMI_message` and the
 * three pure buffer walks came from the leaf pass; the batch after it
 * added the three unpackers and the two ring writers; this one adds
 ...
 * are not written -- but create, control, process and status are read
 * below for the layout, because they are what establishes the sizes and
 * the initial values, and every field's evidence is quoted with it.
 *
 * The pack side is the unpack side's mirror, and that is a structural
 * fact rather than a resemblance (F9190). `framer` carries two bit
 * engines of four fields each, at +0x10..+0x1c and +0x20..+0x2c, and
 * `FAXVMI_create` initialises the two identically -- mask 0, word -1,
 * acc -1, bit 0 at 0x951d7..0x95201. The packers use the first and the
 * unpackers the second, so they are spelled `pack_*` and `unpack_*` here.
 */
 ...
 * correct; what the two halves have between them is more evidence than
 * either had alone, so whoever unifies them should carry these over:
 *
 ...
 * `sysdep_malloc(0x58)` at 0x953c0, so it is 88 bytes and the layout below
 * is closed rather than open-ended.
 *
 * It is one object carrying several unrelated things, which is why it
 * looks odd:
 *
 ...
 * readings agree, which is what makes `max_frame` a name rather than a
 * guess.
 *
 ...
 * states. Their initial values from FAXVMI_create are quoted because that
...
struct faxvmi_framer {
	...
	unsigned short rd;		/* +0x06 READ cursor, in elements.
					 ...
					 * the writers do with `wr`. Named
					 * from a format string, not usage:
					 * faxvmi_hdlc_frame's debug line
					 * "HDLC Transmitted Frame (Length =
					 * %d, iR = %d, iW = %d)" names this
					 * `iR` and the next field `iW`
					 * (F9191)                           */
	unsigned short wr;		/* +0x08 write cursor, in elements;
					 * `iW` in the debug line above      */
	...
	unsigned int unpack_mask;	/* +0x20 the bit being taken out of
					 ...
					 * element". 32 bits wide, held in an
					 * `unsigned short` local by every
					 * reader -- movzwl in, movl out
					 * (F9012)                           */
	...
	short pack_frame_left;		/* +0x46 octets of the frame
					 ...
					 * loads and stores it that way
					 * throughout.  create and control
					 * zero it                           */
 ...
 * `buf` is the element buffer both directions use -- see its own field
 * comment below (F9194).
 *
 ...
 * are `movzwl` loads of sixteen bits at those two offsets, four bytes
 * apart. Neither is written by FAXVMI_create -- `vxx_create` fills them,
 * and this file does not know what it puts there.
 */
struct faxvmi_link {
	...
	unsigned short *buf;		/* +0x04 sysdep_malloc(0x64) -- 50
					 ...
					 * the packers' output: called `rx`
					 * while only the unpackers had been
					 * read, retired now that both
					 * directions are known (F9194).  One
					 * faxvmi instance wraps one
					 * modulation and a slot is either
					 * transmit or receive, so a given
					 * instance only ever uses it one way
					 * round                             */
 ...
 * caller's argument or, when that is null, from `FAXVMI_CFG` (.rodata
 * 0x9490, 0x18 bytes). +0x18 onwards is runtime state.
 */
struct faxvmi {
	...
	unsigned short max_frame;	/* +0x0a the longest frame, in
					 ...
					 * readings agree (F9011, F9019): the
					 * ring is sized to hold it plus
					 * three, the three unpackers stop
					 * producing at it, and
					 * FAXVMI_process reports "full" when
					 * the ring's free space falls below
					 * it                                */
	...
	int underrun;			/* +0x18 the transmit ring ran dry.
					 ...
					 * the async one.  faxvmi_write_fifo
					 * and faxvmi_write_frame clear it
					 * when a whole request is accepted;
					 * create sets it to 1 because nothing
					 * has been written yet; FAXVMI_status
					 * copies it to the status record's
					 * +0x08 and FAXVMI_process makes it
					 * bit 24 of the status word.  Named
					 * from seven sites agreeing on one
					 * reading (F9195): the author's own
					 * phrase for the condition is in the
					 * transmit modulations' message
					 * tables -- "ERROR: Transmit Input
					 * Queue Under-run", code 8 of each --
					 * so the word is the object's even
...
/**
 * `vxx_create[slot]`, `.rodata` 0x9620 -- the first of the six 13-slot
 * tables in address order and the one #FAXVMI_create itself dispatches
 ...
 * *` (`faxadapt.h`). The table's own caller passes `vmi->int_0010` -- plain
 * `int` -- straight into the argument slot with no type-specific handling
 * (`FAXVMI_create`, 0x952be..0x952cf), so the table's element type is the
 * untyped form and the eight typed entries need the same kind of explicit
 * cast `vxx_status` already carries for its own typed four.
 */
typedef void (*faxvmi_create_fn)(struct faxvmi_link *dp, const void *cfg);
extern faxvmi_create_fn const vxx_create[13];

/**
 * @brief Construct (or reinitialise) a Virtual Modem Interface instance.
 *
 * `.text` 0x095120, 705 bytes -- the first of the five entry points in
 * address order. Two behaviours share one function:
 *
 *   `vmi == NULL`   a fresh create.  `vmi`, `vmi->framer` and `vmi->link`
 *                   are allocated here (`sysdep_malloc(0x2c)`/`(0x58)`/
 *                   `(0x18)`, 0x953ab/0x953c0/0x953d4) and every heap block
 *                   below is allocated too.
 *   `vmi != NULL`   a reinit of an existing instance.  Nothing is
 *                   (re)allocated; the ring, the frame buffer and the
 *                   link's two buffers are cleared/refilled in place
 *                   instead.
 *
 * `cfg == NULL` copies #FAXVMI_CFG over `vmi`'s leading six dwords instead
 * of the caller's own (0x95376..0x953a6); both arms rejoin at the exact
 * same six stores (0x95141..0x95160), which is why they are written here as
 * one assignment from a selected source rather than as two branches.
 *
 * The ring's capacity is `max(cfg->fifo_size, cfg->max_frame + 3)`
 ...
 *
 * @param vmi  An existing instance to reinitialise, or NULL to allocate one.
 * @param cfg  Configuration to apply, or NULL for #FAXVMI_CFG.
 * @return The instance (`vmi`, or the newly allocated one).
 */
struct faxvmi *FAXVMI_create(struct faxvmi *vmi, const struct faxvmi_cfg *cfg);

/** The dispatch contract every vxx_* table entry follows for a message
 * lookup: (handle, code, out-parameter). The message form answers a
 * string or NULL. */
...
/**
 * @brief Ask the wrapped modem for a message string.
 * @param vmi   The instance.
 * @param code  A message code; really an unsigned char in the object (it
 *              is loaded with movzbl) where the table entries take an int
 *              -- the narrowing is the author's.
 * @return The message string, or NULL for a code the modulation does not
 *         name.
 */
...
/**
 * @brief Tear down a Virtual Modem Interface instance.
 *
 * Releases the wrapped modulation, then every heap block #FAXVMI_create
 * allocated, then `vmi` itself. Not yet written; declared here (evidence
 * from `FAXVMI_delete`, 0x0953f0) because it establishes layout the other
 * four functions depend on -- see the top of faxvmi.c for what still
 * blocks it.
 *
 * @param vmi  The instance to free.
 */
 ...
 * unchanged to `vxx_status[vmi->slot]` as its own second argument when it
 * is non-NULL -- a caller wanting the wrapped modulation's own status
 * pre-loads a pointer for it there. Every other field is written
 * unconditionally.
 */
...
/**
 * @brief Fill a status record from a Virtual Modem Interface instance.
 *
 * Fills `status` from `vmi`'s own counters and, if `status->modem_status`
 * is set on entry, from the wrapped modulation's own status too -- in
 * which case the return value is that call's return, not 0.
 *
 * @param vmi     The instance.
 * @param status  Destination; `status->modem_status`, if set on entry, is
 *                forwarded to the wrapped modulation's own status call.
 * @return 0 normally; the wrapped modulation's own status return when
 *         `status->modem_status` was set; -1 if `status == NULL` (nothing
 *         touched).
 */
 ...
 * not yet written (this wave's brief excludes it -- it needs
 * `SetScramblerV27`, `TxHdxABV17` and dozens more that two concurrent
 * strands are writing this wave), but `FAXVMI_CTL` is a plain 24-byte
 * `.rodata` object with no dependency of its own, and its type is what
 * this file needs to declare it.
 *
 ...
 * literal -1 (confirmed against `dis.py`, F10108). Four of the six real
 * fields already have a habitable name (the struct carried eight members
 * before the pad-region removal audit, F10145, took out `pad_000a` and
 * `pad_0012` -- both were the compiler's own alignment gap ahead of
 * `int_000c`/`int_0014`, confirmed by the CTL_ASSERT_OFF entries below
 * still holding); the rest are usage inference only, hedged as such, and
 * left neutral rather than guessed further -- CLAUDE.md's "naming wrongly
 * is worse than padding" ground.
 */
...
/**
 * @brief Apply a control record to a Virtual Modem Interface instance.
 *
 * `.text` 0x095650, 338 bytes. Applies `ctl`'s effects to `vmi` in the
 * object's own order:
 *
 *   1. `ctl == NULL` returns -1 immediately, nothing touched.
 *   2. `ctl->int_0014` nonzero recurses FIRST, before anything else
 *      applies: `vxx_control[vmi->slot](vmi->link,
 *      (void *)(long)ctl->int_0014)`, and that call's return becomes this
 *      function's own return (0 otherwise) -- confirmed against `dis.py`,
 *      matching F10108's own trace of this same site.
 ...
 *      `FAXVMI_create` itself uses; `zero_run_bits` = 0 -- overwritten
 *      again at step 5 below if `ctl->short_0008` also applies;
 *      `unpack_mask` = 0, `unpack_word`/`unpack_acc` = -1, `async_hunt` = 1,
 *      `zero_run_send` = 0, `zero_run_seen` = 0, `pack_mask` = 0,
 *      `pack_word`/`pack_acc` = -1) -- and sets `vmi->mode =
 *      ctl->short_0010`.
 ...
 *
 * @param vmi  The instance.
 * @param ctl  The control record, or NULL (returns -1, nothing touched).
 * @return See step 6 above.
 */
 ...
 * `short *`, `unsigned short *`, `unsigned short *`) but disagree on which
 * of the last three is `out`/`in` and which is `count`/`result`
 * (`faxadapt.h`'s own comment on the mirrored shapes). `FAXVMI_process`
 * calls through this table without ever caring which is which, so it is
 * written that way here too, and the TX/RX declarations in
 * `faxadapt.h`/`nulldp.h` remain the typed record of what each entry
...
/**
 * @brief Move one block both ways through the wrapped modulation.
 *
 * `.text` 0x095470, 327 bytes. One call moves one block both ways through
 * the wrapped modulation:
 *
 *   1. `vmi->reverse`: `vmi_reverse[mode](data, *count)` -- bit-reverse the
 *      caller's buffer in place before anything else touches it.
 *   2. `n = vmi_pack[mode](vmi, data, *count)` -- frame `*count` elements
 *      of `data` into `vmi->link->buf`; `n` is `link->pack_count`.
 *   3. `ret = vxx_process[slot](vmi->link, pcm, &n, result)` -- drive one
 *      block through the wrapped modem. `n` is passed by reference and the
 *      table's own entries (`faxadapt.h`) both copy it to `*result` and
 *      then zero it, so by step 4 it is 0 for every real modulation and
 *      unchanged (still `link->pack_count`) for the null slot, which never
 *      touches it.
 *   4. `m = vmi_unpack[mode](vmi, data, n)` -- unframe `n` elements back
 *      out of `vmi->link->buf` into `data`. `*count` is then set to `m`.
 ...
 * `count` is read signed on entry (feeds step 2), then overwritten with
 * `m` on exit (0x9549a, 0x954fc).
 *
 * @param vmi     The instance.
 * @param data    The framed/unframed buffer, `*count` elements on entry.
 * @param pcm     Passed through to `vxx_process` as its 2nd argument.
 * @param count   In: elements of `data` to frame; out: elements unframed
 *                back into `data`.
 * @param result  Passed through to `vxx_process` as its 4th argument.
 * @return The composed status word (the FAXVMI_STATUS_* bits above, plus
 *         `vxx_process`'s own low 24 bits and bits 29..31 unchanged).
 */
 ...
 * (`class1tx.c`'s span, `_send_hdlc_buffer_state` at 0x9e4b6 and
 * neighbours) -- defined in `class1.c`, its first writer, not here and not
 * in `faxvmi.c`, on the same footing as `FAXVMI_CTL`.
 */
 ...
 * FIRST, hands the return to `vxx_process` as a sample count, and only
 * then unpacks (0x954ab..0x954f5).
 *
 ...
 * anything derived from `count`. Both write exactly `pack_count` elements
 * of `pack_width` bits whatever the input was; a starved ring is padded
 * with fill rather than shortening the block. `link->buf` is 50 elements
 ...
 * counts only the elements it filled before the ring first ran dry, and
 * the async form returns `pack_count` unconditionally.
 */

/**
 * @brief Simple framing: eight bits per source octet, MSB first, no
 * framing at all.
 *
 ...
 *
 * @param vmi    The instance.
 * @param src    Caller's buffer of `count` elements (fed via the ring;
 *               see the group note above).
 * @param count  Elements offered.
 * @return Elements filled before the ring, if any, first ran dry.
 */
int faxvmi_simp_pack(struct faxvmi *vmi, unsigned short *src, short count);

/**
 * @brief Asynchronous framing: start bit, eight data bits, stop bit.
 *
 * Each source octet becomes a ten-bit character -- a zero start bit, the
 * eight data bits MSB first, then a one stop bit, assembled as
 * `((octet << 1) | 1) & ~0x200` and shifted out from 0x200 down. A ring
 * that runs dry contributes a single MARK bit and sets `vmi->underrun`,
 * which is the idle line.
 *
 * `framer->zero_run_send` diverts it: while that is set, one ZERO bit is
 * sent per call to the refill and `framer->zero_run_bits` counts down.
 *
 * @param vmi    The instance.
 * @param src    Caller's buffer of `count` elements (fed via the ring;
 *               see the group note above).
 * @param count  Elements offered.
 * @return `pack_count`, unconditionally.
 */
int faxvmi_asyc_pack(struct faxvmi *vmi, unsigned short *src, short count);

/**
 * @brief HDLC framing: flags, frame data, and bit stuffing.
 *
 * Three flag octets, then the frame, with a zero inserted after every five
 * consecutive one bits of data. What comes out of the ring is what
 * `faxvmi_write_frame` put in -- one length element, then that many
 * octets, then two FCS octets -- so `framer->pack_frame_left` is what says
 * whether the next element is a length or an octet. An empty ring sends
 * one flag and raises `vmi->underrun`; flags are the idle pattern, so that
 * is not a stall.
 *
 * @param vmi    The instance.
 * @param src    Caller's buffer of `count` elements (fed via the ring;
 *               see the group note above).
 * @param count  Elements offered.
 * @return Elements consumed (see the group note above for the general
 *         contract).
 */
 ...
 * `vmi->link->buf` -- not from a caller's buffer -- and octets are written
 * to `dst`, one octet per 16-bit element, low byte only. The return is how
 * many elements were written to `dst`, and it is a short widened to int.
 *
 ...
 * discarded at the return. All three do this, so it is the interface and
 * not an oversight: the caller re-presents the buffer, and the bit
 * position within an element is what the framer carries across calls
 * (`mask`).
 *
 ...
 * `max_frame` elements; the HDLC form's guard does not accumulate across
 * the frames of one call (D1073) and can write far more. This is
 * F8607/D956's shape and it is called out rather than discovered again.
 */

/**
 * @brief Simple unframing: every eight bits is an octet, MSB first.
 *
 * The first bit taken ends up in bit 7.
 *
 * @param vmi    The instance.
 * @param dst    Destination octets, one per 16-bit element.
 * @param count  Input elements available in `vmi->link->buf`.
 * @return Elements written to `dst` (see the group note above).
 */
int faxvmi_simp_unpack(struct faxvmi *vmi, unsigned short *dst, short count);

/**
 * @brief Asynchronous unframing: hunt for a start bit, then eight data bits.
 *
 * While `framer->async_hunt` is set every one bit is mark and the first
 * zero begins a character; the eight bits after it are the character and
 * the framer goes back to hunting.
 *
 * Two things run on every bit, whatever the state, and both test the low
 * 23 bits of the accumulator: 23 zeros restart the hunt with the bit
 * counter cleared, and 22 zeros followed by a one set
 * `framer->zero_run_seen`.
 *
 * @param vmi    The instance.
 * @param dst    Destination octets, one per 16-bit element.
...
/**
 * @brief HDLC unframing: destuffing, flags, FCS, and a repair pass.
 *
 * Frames are written to `dst` length-prefixed -- one element of length,
 * then that many octets -- which is `faxvmi_frame_reverse`'s layout.
 *
 * A frame whose FCS never comes right is emitted with a length of zero
 * rather than dropped, so the frame count still advances.
 *
 * @param vmi    The instance.
 * @param dst    Destination: length-prefixed frames.
 * @param count  Input elements available in `vmi->link->buf`.
 * @return Elements written to `dst` (see the group note above).
 */
...
/**
 * @brief Copy elements into the framer's ring, one for one.
 * @param vmi    The instance.
 * @param src    In/out: cursor into the caller's buffer, advanced by
 *               however many elements were taken.
 * @param count  Elements offered.
 * @return Elements the ring actually took.
 */
int faxvmi_write_fifo(struct faxvmi *vmi, unsigned short **src, short count);

/**
 * @brief Append length-prefixed frames to the framer's ring, with FCS.
 *
 * Each frame becomes a length element of `len + 2`, the data, and two FCS
 * octets.
 *
 * @param vmi    The instance.
 * @param src    In/out: cursor into the caller's length-prefixed frames,
 *               advanced by however many frames were taken.
 * @param count  Frames offered.
 * @return Frames the ring actually took.
 */
...
/**
 * @brief Compute the HDLC frame check sequence over `count` elements.
 *
 * The generator is 0x1021 -- x^16 + x^12 + x^5 + 1, the CRC-CCITT/T.30
 * polynomial -- with the register initialised to 0xFFFF, fed most
 * significant nibble first, and complemented on the way out. That is read
 * off the instructions and not guessed: each of the two steps per element
 * computes `n = (fcs ^ (octet << k)) >> 12` and then
 *
 ...
 * #faxvmi_byte_reverse.
 *
 * @param buf    Elements to check, one octet each (low byte only).
 * @param count  Elements to process.
 * @return The complemented CRC-CCITT/T.30 FCS.
 */
int faxvmi_gen_fcs16(unsigned short *buf, short count);

/**
 * @brief Reverse the low eight bits of each of `count` elements, in place.
 * @param buf    Elements to reverse, one octet each (low byte only).
 * @param count  Elements to process.
 */
void faxvmi_byte_reverse(unsigned short *buf, short count);

/**
 * @brief Bit-reverse the data octets of `count` length-prefixed frames.
 *
 * Frames are laid end to end -- one element of length, then that many data
 * elements -- and #faxvmi_byte_reverse is run over each frame's data. The
 * length element itself is not reversed.
 *
 * @param buf    Length-prefixed frames.
 * @param count  Frames to process.
 */
void faxvmi_frame_reverse(unsigned short *buf, short count);

/* --------------------------------------------------------------------- */
/*
 * The three dispatch tables, indexed by `vmi->mode`. They carry the
 * author's own names in the object's symbol table -- `vmi_unpack` at
